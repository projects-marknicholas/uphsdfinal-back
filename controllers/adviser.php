...
class AdviserController{
  public function get_account_by_id() {
    ...
    if ($security_response['role'] !== 'adviser' && $security_response['role'] !== 'authorized_user') {
  ...
  public function get_types() {
    ...
    // Variables
    $scholarship_type_id = 'bb32127037342ec9c79ac4a49ac58b99';
  ...
  public function insert_form_attachment() {
    ...
    if (!in_array($security_response['role'], ['adviser', 'authorized_user'])) {
      echo json_encode(['status' => 'error', 'message' => 'Unauthorized access']);
      return;
    }
  
    // Validate referral_id exists as a user_id with role 'adviser'
    if (!empty($referral_id)) {
      $check_referral_id = $conn->prepare("SELECT user_id FROM users WHERE user_id = ? AND role = 'adviser'");
      ...
      if ($check_referral_id->num_rows === 0) {
        $response['status'] = 'error';
        $response['message'] = 'Invalid referral ID or the referred user is not a adviser.';
        echo json_encode($response);
    ...
    if ($check_student_number->num_rows === 0) {
      error_log("No matching student number found for: {$student_number}");
      $response['status'] = 'error';
    ...
    if ($current_date < $start_date || $current_date > $end_date) {
      $response = [
        'status' => 'error',
        'message' => 'Application period is not active.'
      ];
      echo json_encode($response);
      exit;
    }
    ...
    if (!$query_category) {
      $response = [
        'status' => 'error',
        'message' => 'Failed to prepare category query.',
        'error' => $conn->error
      ];
      echo json_encode($response);
      exit;
    ...
    if (empty($category)) {
      $response = [
        'status' => 'error',
        'message' => 'Category not found for the given scholarship_type_id.'
      ];
      echo json_encode($response);
    ...
    if (!$check_duplicate_application) {
      $response = [
        'status' => 'error',
        'message' => 'Failed to prepare duplicate application query.',
        'error' => $conn->error
      ];
      echo json_encode($response);
      exit;
    ...
    if ($check_duplicate_application->num_rows > 0) {
      $response = [
        'status' => 'error',
        'message' => 'You already applied within this type or category.'
      ];
      echo json_encode($response);
      $check_duplicate_application->close();
      exit;
  ...
  public function get_user_accounts() {
    ...
    if ($security_response['role'] !== 'adviser') {
      echo json_encode(['status' => 'error', 'message' => 'Unauthorized access']);
      return;
    }
  ...
  public function get_department() {
    ...
    // Check if the user's role is 'adviser'
    if ($security_response['role'] !== 'adviser') {
      echo json_encode(['status' => 'error', 'message' => 'Unauthorized access']);
      return;
    }
